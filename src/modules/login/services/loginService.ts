...
export class LoginService {
  ...
  constructor(
    private readonly loginRequester: typeof sendLoginRequest = sendLoginRequest,
    private readonly logoutRequester: typeof sendLogoutRequest = sendLogoutRequest,
  ) {
    makeObservable(this);
  ...
  async logout() {
    ...
    try {
      if (refreshToken) {
        await this.logoutRequester(refreshToken);
      }
    } catch (error) {
      if (import.meta.env.DEV) {
        // eslint-disable-next-line no-console -- Helpful for diagnosing logout issues during development.
        console.warn('Failed to revoke tokens during logout.', error);
      }
    } finally {
      clearAuthTokens();
      this.isSubmitting = false;
